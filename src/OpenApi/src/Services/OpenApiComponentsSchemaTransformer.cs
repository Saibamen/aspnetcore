                ...
                if (pathItem.Operations.TryGetValue(operationType, out var operation))
                {
                    if (operation.Parameters is not null)
                    {
                        foreach (var parameter in operation.Parameters)
                        {
                            ...
                            {
                                parameter.Schema = new OpenApiSchema { Reference = new OpenApiReference { Type = ReferenceType.Schema, Id = "#" } };
                            }
                    ...
                    if (operation.RequestBody is not null)
                    {
                        foreach (var content in operation.RequestBody.Content)
                        {
                            if (content.Value.Schema != null)
                            {
                                ...
                                {
                                    if (property.Value.Reference is { Id: "#" })
                                    {
                                        property.Value.Reference = new OpenApiReference { Type = ReferenceType.Schema, Id = "#" };
                                    }
                                }
                            }
                            ...
                            if (content.Value.Schema != null && content.Value.Schema.AnyOf != null)
                            {
                                for (var j = 0; j < content.Value.Schema.AnyOf.Count; j++)
                                {
                                    if (content.Value.Schema.AnyOf[j].Extensions.TryGetValue(OpenApiConstants.SchemaId, out var allOfSchemaIdExtension) && allOfSchemaIdExtension is OpenApiString { Value: string allOfSchemaId })
                                    {
                                        content.Value.Schema.AnyOf[j] = new OpenApiSchema { Reference = new OpenApiReference { Type = ReferenceType.Schema, Id = allOfSchemaId } };
                                    }
                                }
                            }
